...
final class HybridCompressionStoredFieldsConsumer extends StoredFieldsConsumer {
    ...
    private final StoredFieldsFormat TEMP_STORED_FIELDS_FORMAT =
            new Lucene90CompressingStoredFieldsFormat(
                    "HybridStoredFields", NO_COMPRESSION, 128 * 1024, 1, 10);
    TrackingTmpOutputDirectoryWrapper tmpDirectory;

    HybridCompressionStoredFieldsConsumer(Codec codec, Directory directory, SegmentInfo info) {
...
//        if (state.context.flushInfo != null && state.context.flushInfo.estimatedSegmentSize < 16 * 1024 * 1024){
//            IOUtils.deleteFiles(tmpDirectory, tmpDirectory.getTemporaryFiles().values());
//            return;
//        }

        infoStream.message("hybridStoredFieldsConsumer", "writes happening again");
